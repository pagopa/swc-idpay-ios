...
extension TransactionHistoryListUITests {
    
    private func loadMockedHistoryList() {
        app.launchEnvironment = ["-mock-filename": "TransactionsHistoryList"]
        app.signIn(success: true)
        app.openMenuSection("Storico operazioni")
    }
    
    private func loadEmptyHistoryList() {
        app.launchEnvironment = ["-empty-state": "1"]
        app.signIn(success: true)
        app.openMenuSection("Storico operazioni")
    }
}
